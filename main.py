...
class UiImages:
    ...
    beginImage = pygame.image.load(
        "./assets/images/UI/BEGIN AND END/GET READY.png"
    ).convert_alpha()
    endImage = pygame.image.load(
        "./assets/images/UI/BEGIN AND END/LEVEL COMPLETE.png"
    ).convert_alpha()

    # ERRORS
    error_title = pygame.image.load(
        "./assets/images/UI/errors/error_occurred.png"
    ).convert_alpha()
    ...
    pause = [
        pygame.transform.scale(
            pygame.image.load("./assets/images/UI/pause/paus'd (1).png"), (450, 150)
        ).convert_alpha(),
        pygame.transform.scale(
            pygame.image.load("./assets/images/UI/pause/paus'd (2).png"), (450, 150)
        ).convert_alpha(),
        pygame.transform.scale(
            pygame.image.load("./assets/images/UI/pause/paus'd (3).png"), (450, 150)
        ).convert_alpha(),
    ]
    ...
    quit_button_pressed = pygame.image.load(
        "./assets/images/UI/quit_button_pressed.png"
    ).convert_alpha()
    ...
    start_button_pressed = pygame.image.load(
        "./assets/images/UI/start_button_pressed.png"
    ).convert_alpha()
    START = [start_button, start_button_pressed]
    del start_button, start_button_pressed

    options_button = pygame.image.load(
        "./assets/images/UI/options_button.png"
    ).convert_alpha()
    options_button_pressed = pygame.image.load(
        "./assets/images/UI/options_button_pressed.png"
    ).convert_alpha()
    OPTIONS = [options_button, options_button_pressed]
    del options_button, options_button_pressed

    continue_button = pygame.image.load(
        "./assets/images/UI/continue_button.png"
    ).convert_alpha()
    continue_button_pressed = pygame.image.load(
        "./assets/images/UI/continue_button_pressed.png"
    ).convert_alpha()
    ...
    yes = pygame.transform.scale(
        pygame.image.load("./assets/images/UI/YES.png"), (50, 25)
    ).convert_alpha()
    no = pygame.transform.scale(
        pygame.image.load("./assets/images/UI/NO.png"), (50, 25)
    ).convert_alpha()
    ...
    # SETTINGS IMAGES
    option_bg = pygame.transform.scale(
        pygame.image.load("./assets/images/UI/options/bg.png"), (450, 600)
    ).convert_alpha()
    option_30 = pygame.transform.scale(
        pygame.image.load("./assets/images/UI/options/FPS30.png"), (50, 30)
    ).convert_alpha()
    option_60 = pygame.transform.scale(
        pygame.image.load("./assets/images/UI/options/FPS60.png"), (50, 30)
    ).convert_alpha()

    # BAGKGROUNDS
    title_bg = pygame.transform.scale(
        pygame.image.load("./assets/images/UI/title/background.png"), (450, 600)
    ).convert_alpha()
    error_bg = pygame.transform.scale(
        pygame.image.load("./assets/images/UI/errors/bg.png"), (450, 600)
    ).convert_alpha()
    load_bg = pygame.transform.scale(
        pygame.image.load("./assets/images/UI/splash.png"), (450, 600)
    ).convert_alpha()
    ...
    # GAME OVER
    game_over_sign = pygame.image.load(
        "./assets/images/UI/GAME OVER.png"
    ).convert_alpha()
...
for directory in os.listdir("./assets/images/bg/"):
    ...
    if "." in directory:
        continue  # breaking if there is a file extension, as it should only be detecting folders
    # removing empty directories
    if len(os.listdir("./assets/images/bg/" + directory)) <= 0:
        continue
    ...
    # loading each folder
    loaded_backgrounds[str(directory)] = [
        None,
        [],
    ]  # index 0 is config file, index 1 contains the imported images
    ...
    if (
        (loaded_backgrounds[directory][0] == None)
        or ("resize" not in loaded_backgrounds[directory][0])
        or (loaded_backgrounds[directory][0]["resize"] == 3)
    ):
    ...
    while True:
        try:
            ...
            if resize == 0:
                ...
                resize == 1
            ):  # scaling vertically based off what needs to be scaled horizontally to fit the screen
                multiplier = 450 / item.get_width()
                scale = (item.get_width() * multiplier, item.get_height() * multiplier)
                del multiplier
            elif (
                resize == 2
            ):  # scaling horizontally based off what needs to be scaled vertically to fit the screen
            ...
            loaded_backgrounds[directory][1].append(
                item
            )  # adds image to main directory
...
sounds = Sounds()
sounds.apply_offsets()  # this creates an object of the class. this is what everything refers to
...
# 0-9
for _ in range(10):
    loaded_text[_] = pygame.font.Font("./assets/font/Terminus.ttf", 20).render(
        str(_), True, "white", "black"
    )
# world names
...
for _ in settings.keys():
    loaded_text[_] = pygame.transform.scale(
        pygame.font.Font("./assets/font/Setfont-Regular.ttf", 50).render(
            str(_), True, "white"
        ),
        (75, 40),
    )
...
def draw_text(
    ...
    if shadow:
        score_surface = score_font.render(str(score_value), True, color, shadow_color)
    else:
        score_surface = score_font.render(str(score_value), True, color)

    score_rect = score_surface.get_rect()

    if snap and snap_direction == "r":
        score_rect.right = coord[0]
        score_rect.y = coord[1]
    elif snap and snap_direction == "l":
        score_rect.left = coord[0]
        score_rect.y = coord[1]
    else:
        score_rect.center = coord
...
class BG:
    ...
    def update(self, scroll_values: tuple = None):
        ...
        if self.bgY >= self.dir[self.curFrame].get_height() or self.bgY <= (
            self.dir[self.curFrame].get_height() * -1
        ):
            self.bgY = 0
        if self.bgX >= self.dir[self.curFrame].get_width() or self.bgX <= (
            self.dir[self.curFrame].get_width() * -1
        ):
            self.bgX = 0
    ...
    def decoys(self):
        if self.bgY != 0:
            window.blit(
                self.dir[self.curFrame],
                (self.bgX, self.bgY - self.dir[self.curFrame].get_height()),
            )
            window.blit(
                self.dir[self.curFrame],
                (self.bgX, self.bgY + self.dir[self.curFrame].get_height()),
        ...
        if self.bgX != 0:
            window.blit(
                self.dir[self.curFrame],
                (self.bgX + self.dir[self.curFrame].get_width(), self.bgY),
            )
            window.blit(
                self.dir[self.curFrame],
                (self.bgX - self.dir[self.curFrame].get_width(), self.bgY),
        ...
        if self.bgY != 0 and self.bgX != 0:
            window.blit(
                self.dir[self.curFrame],
                (
                    self.bgX + self.dir[self.curFrame].get_width(),
                    self.dir[self.curFrame].get_height() - 600,
                ),
            )
            window.blit(
                self.dir[self.curFrame],
...
class Level:
    def __init__(self, player, world_num):
        ...
        self.world_num_looped = self.world_num - (
            self.world_num * (self.world_num // len(self.worldOrder))
        )
    ...
    def refresh_world_data(self):
        ...
        if self.form is None:
            self.form = Formation(
                player=self.player, level=self.level, file=self.world_file
            )
        else:
            self.form.__init__(
                player=self.player, level=self.level, file=self.world_file
    ...
    def advance_world(self):
        self.world_num += 1
        self.world_num_looped = self.world_num - (
            len(self.worldOrder) * ((self.world_num - 1) // len(self.worldOrder))
        )
...
def title():
    ...
    def redraw_window():
        ...
        window.blit(
            ui.title_bg_logo[frame],
            (225 - (ui.title_bg_logo[frame].get_width() / 2), 25),
        )
    ...
    while run:
        ...
        if end - start >= 0.25:
            start = time.time()
            frame += 1
            if frame > (len(ui.title_bg_logo) - 1):
                frame = 0
        ...
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_a:
                    ...
                    key_index = list(settings)[index]
                    if (
                        settings[key_index][1] == "slider"
                        and settings[key_index][0] > 0
                    ):
                ...
                if event.key == pygame.K_d:
                    ...
                    # HARD-CODING PART FOR MUTE CODE. ERASE IF RE-USED.
                    if key_index == "OST VOL" or key_index == "SOUND VOL":
                        settings["MUTE"][0] = False

                    # applying settings
                    if (
                        settings[key_index][1] == "slider"
                        and settings[key_index][0] < 1
                    ):
                        settings[key_index][0] += 0.05
                        settings[key_index][0] = round(settings[key_index][0], 2)
                    else:
...
def pause(
    img=None,
):  # it takes in the UI images as an argument to lower RAM usage and prevent leakage :)
    #-----PLEASE NOTE !-----
    ...
    def display_update():
        ...
        window.blit(
            ui.pause[graphic_frame],
            ((225 - (ui.pause[graphic_frame].get_width() / 2)), 100),
        )

        if index == 1:
            window.blit(ui.CONTINUE[1], ((225 - (ui.CONTINUE[1].get_width() / 2)), 275))
        else:
            window.blit(ui.CONTINUE[0], ((225 - (ui.CONTINUE[0].get_width() / 2)), 275))
        if index == 2:
            window.blit(ui.OPTIONS[1], ((225 - (ui.OPTIONS[1].get_width() / 2)), 375))
        else:
            window.blit(ui.OPTIONS[0], ((225 - (ui.OPTIONS[0].get_width() / 2)), 375))
        if index == 3:
            window.blit(ui.QUIT[1], ((225 - (ui.QUIT[1].get_width() / 2)), 475))
        else:
            window.blit(ui.QUIT[0], ((225 - (ui.QUIT[0].get_width() / 2)), 475))
        pygame.display.update()
    ...
    while run:
        ...
        if graphic_time >= 0.25 and graphic_frame < 2:
            graphic_frame += 1
            graphic_start = time.time()
        elif graphic_time >= 0.25 and graphic_frame >= 2:
            graphic_frame = 1
            graphic_start = time.time()
        else:
            pass
...
def game_over(level_class: Level = None, score: int = 0):
    ...
    quote = random.choice(
        [
            "sheesh",
            "you suck",
            "BURN DOWN THE MCDONALD",
            "nop\ne",
            "is your monitor on",
            "i don't know what to put on the screen",
            "get better",
            "greasy grove",
            "ahhhhhhhhhhhhhh",
            "computer aids cheese",
            "i love men",
            "POOPENFARTEN ",
            "FORT NITE gntals",
            "this is a sentence",
            ...
            "TOOTH PASTE !",
            "JOHNNY BALLSICKLE",
            "DO NOT THROW BANANAS AT GOATS",
            "I AM THE MASTER ABLEIST",
            "COMMUNISM",
            "CAPITALISM",
            "ANARCHISM",
            "CLOROX SUCKS",
            "KRUEGER CLICKER",
            "WHERE IS MY WIFE?",
            "PROMORTOIJNNSOFKJAAS:LFAJFEO:DFKJIHADFH",
            "?????",
            "EDGY WOLF TEEN MEETS BAND KID",
            "TOXIC MASCULINITY",
            "SQUID",
            "THE LOBOTOMY IS CONFIRMED",
    ...
    scroll_values = [
        level_class.bg.config["scrollHoriz"],
        level_class.bg.config["scrollVert"],
    ]
    ...
    while run:
        ...
        if state == "slowdown":
            ...
            if level_class.bg.config is not None and (
                scroll_values[1] > 0.1 or scroll_values[0] > 0.1
            ):
            ...
            elif level_class.bg.config is None or (
                scroll_values[1] <= 0.1 or scroll_values[0] <= 0.1
            ):
            ...
            if state_results == 0 or state_results == 3:
                ...
                score_counter = (
                    score_counter + int(round(((score - score_counter) / 50), 0))
            ...
            window.blit(
                ui.game_over_sign, (center, 0.05 * ((game_over_image_x - 200) ** 2))
            )
        # handling EXITING
        for event in pygame.event.get():
            if event == pygame.QUIT:
...
def level_complete(
    level_class: Level = None, player: loaded_characters["player"].Player = None
):
    # startup info
    FPS = 60
    run = True
    ...
    if player.health > 1:
        inspirational_list.append("YOU DID GREAT!")
    else:
        inspirational_list.append("YOU SUCK!")
    #The world name has an error where, if you look past the range of the worlds, an error is raised
    #this will prevent that.#
    next_world_number = (
        0
        if level_class.world_num_looped > (len(level_class.worldOrder) - 1)
        else level_class.world_num_looped
    )
    ...
    while run:
        ...
        if state == "speedup" and speedups_applied < 256:
            ...
            scroll_values = (
                [scroll_values[0] * 1.025, scroll_values[1] * 1.025]
                if scroll_values[0] < 100 and scroll_values[1] < 100
                else scroll_values
            )
            ...
            if speedups_applied == 112:
                text = Text(
                    ...
                    pattern="static sine",
                    speed=1,
                    text=str(level_class.worldOrder[next_world_number]),
                )
                groups["universal"].add(text)
                groups["priority"].add(text)
            # 4
        ...
        # SLOWDOWN
            ...
            abs(scroll_values[0] - level_class.bg.config["scrollHoriz"]) > 1
            or abs(scroll_values[1] - level_class.bg.config["scrollVert"]) > 1
        ):
            scroll_values = [scroll_values[0] * 0.95, scroll_values[1] * 0.95]
        # RECENTERING THE BACKGROUND
        elif state == "slowdown" and (
            abs(level_class.bg.bgX) > 10 or abs(level_class.bg.bgY) > 10
        ):
            # RECENTERING BG
...
def play(bullet_shared=loaded_bullets["shared"]):
    ...
    player = loaded_characters["player"].Player(
        sounds=sounds, loaded_bullets=loaded_bullets, groups=groups
    )
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_i or event.key == pygame.K_ESCAPE:
                    ...
                    sounds.play_song(
                        str(level_class.world_file.worldInfo["songname"]), pos=time_pos
                    )
