...
class img():
    ...
    for i in range(len(bullet)):bullet[i]=pygame.transform.scale(bullet[i],(10,10)).convert_alpha()
...
class SPIKEBULLETS(pygame.sprite.Sprite):
    ...
    def update(self):
        if self.direction == 0: self.rect.x -= 10
        if self.direction == 1: self.rect.x += 10
        if self.direction == 2: self.rect.y -= 10
        if self.direction == 3: self.rect.y += 10

        if self.rect.right <= 0 or self.rect.left >= 600 or self.rect.top <= 0 or self.rect.bottom >= 800: self.kill()
