...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self, HurtSprites ,bulletsprites,allsprites,sounds):
        ...
        #imagerect is the rectangle of the image
        #hitboxrect is the rectangle used for detecting player location and such.
        #what is considered the "official" rect will change on occasion.
        #This is because a pygame sprite will constantly check for a "self.rect" asset, which there can only be one of.
        #Therefore, when imagerect is needed, self.rect becomes imagerect.
        #   When hitboxrect is needed, self.rect becomes hitboxrect
        self.imageRect = self.image.get_rect()
        self.hitboxRect=self.imageRect.inflate(-30,-30);self.hitboxRect.center = self.imageRect.center
    ...
    def controls(self, event):
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_SPACE and self.playerdied == False:
    ...
    def collision(self):
        self.rect = self.imageRect
        self.hitboxRect.center = self.imageRect.center

        # YUP COLLISION WITH WALLS
        #self.rect is not used here, since both are being compared through non-pygame means.
        """WHY IS THE IMAGE BEING MOVED FURTHER OFFSCREEN?
        HitboxRect: positioning, collision
        ImageRect: drawing picture to screen
        imageRect is larger than hitboxRect, so if imageRect is moved back onscreen, the character would jitter heavily
        """
        if self.hitboxRect.left >= 0 and self.hitboxRect.right <= 600:
            self.imageRect.x += self.player_x
        elif self.hitboxRect.left <= 0:
            self.imageRect.left = -15
        elif self.hitboxRect.right >= 600:
            self.imageRect.right = 615
        if self.hitboxRect.top >= 0 and self.hitboxRect.bottom <= 800:
            self.imageRect.y += self.player_y
        elif self.hitboxRect.top <= 0:
            self.imageRect.top = -15
        elif self.hitboxRect.bottom >= 800:
            self.imageRect.bottom = 815

        # YUP COLLISION WITH ENEMIES
        #hitbox is needed for collision, so hitbox becomes rect
        self.rect = self.hitboxRect
        Hit = pygame.sprite.spritecollide(self, self.HurtSprites, False)
        ...
        if self.lives <= 0:
            ...
            self.deathCoord = (self.imageRect.center)
        #imagerect is needed near the end, since this is what is used when pygame BLITS the image
        self.rect = self.imageRect
    def bulletmanager(self,event):
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_b and self.playerdied == False:
                self.invenIndex += 1
                self.sounds.select.play()
    def autoshoot(self):
        #funny
        # shoot((self.rect.x + 20), (self.rect.y + 20), self.allsprites, self.bulletsprites, self.HurtSprites, "missile")
        # shoot((self.rect.x + 20), (self.rect.y + 20), self.allsprites, self.bulletsprites, self.HurtSprites, "bees")
        # shoot((self.rect.x + 20), (self.rect.y + 20), self.allsprites, self.bulletsprites, self.HurtSprites, "default")
